...
LANGUAGE sql STABLE SET search_path = public, extensions, extensions AS $$
  ...
  SELECT * FROM (
    ...
    WHERE i.name ILIKE '%'||(SELECT n FROM needle)||'%' OR coalesce(i.abbreviation,'') ILIKE '%'||(SELECT n FROM needle)||'%' OR similarity(i.name,(SELECT n FROM needle)) > 0.25
    UNION ALL
    ...
    FROM public.researchers r LEFT JOIN public.institutions i ON i.id = r.institution_id
    WHERE r.full_name ILIKE '%'||(SELECT n FROM needle)||'%' OR similarity(r.full_name,(SELECT n FROM needle)) > 0.25
    UNION ALL
    ...
    WHERE t.name ILIKE '%'||(SELECT n FROM needle)||'%' OR similarity(t.name,(SELECT n FROM needle)) > 0.25
    UNION ALL
    ...
    FROM public.opportunities o LEFT JOIN public.institutions i ON i.id = o.institution_id
    WHERE o.title ILIKE '%'||(SELECT n FROM needle)||'%' OR similarity(o.title,(SELECT n FROM needle)) > 0.25
    UNION ALL
    ...
    FROM public.projects p LEFT JOIN public.institutions i ON i.id = p.institution_id
    WHERE p.name ILIKE '%'||(SELECT n FROM needle)||'%' OR coalesce(p.acronym,'') ILIKE '%'||(SELECT n FROM needle)||'%' OR similarity(p.name,(SELECT n FROM needle)) > 0.25
    UNION ALL
    ...
    WHERE pb.title ILIKE '%'||(SELECT n FROM needle)||'%' OR similarity(pb.title,(SELECT n FROM needle)) > 0.3
    UNION ALL
    ...
    WHERE e.title ILIKE '%'||(SELECT n FROM needle)||'%' OR similarity(e.title,(SELECT n FROM needle)) > 0.25
...
BEGIN
  ...
  SELECT t.id,
    ...
    now()
  FROM public.research_topics t
  LEFT JOIN (SELECT pt.topic_id, count(*) c FROM public.publication_topics pt JOIN public.publications p ON p.id = pt.publication_id
             WHERE p.publication_date >= (current_date - interval '12 months') GROUP BY 1) p12 ON p12.topic_id = t.id
  LEFT JOIN (SELECT pt.topic_id, count(*) c FROM public.publication_topics pt JOIN public.publications p ON p.id = pt.publication_id
             WHERE p.publication_date >= (current_date - interval '24 months') AND p.publication_date < (current_date - interval '12 months') GROUP BY 1) pprev ON pprev.topic_id = t.id
  LEFT JOIN (SELECT pt.topic_id, count(*) c FROM public.publication_topics pt JOIN public.publications p ON p.id = pt.publication_id
             WHERE p.publication_date >= (current_date - interval '36 months') GROUP BY 1) p36 ON p36.topic_id = t.id
  LEFT JOIN (SELECT prt.topic_id, count(*) c FROM public.project_topics prt JOIN public.projects pj ON pj.id = prt.project_id
             WHERE pj.status = 'active' GROUP BY 1) pr ON pr.topic_id = t.id
  LEFT JOIN (SELECT ot.topic_id, count(*) c FROM public.opportunity_topics ot JOIN public.opportunities o ON o.id = ot.opportunity_id
             WHERE o.status IN ('open','closing_soon','rolling') GROUP BY 1) op ON op.topic_id = t.id
  LEFT JOIN (SELECT it.topic_id, count(DISTINCT it.institution_id) c FROM public.institution_topics it GROUP BY 1) inst ON inst.topic_id = t.id
