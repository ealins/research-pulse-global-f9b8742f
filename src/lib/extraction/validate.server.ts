...
const OPPORTUNITY_TYPES = ["phd", "doctoral_researcher", "research_assistant", "postdoc", "other"] as const;
...
export function validateVacancy(completion: string): ValidationOutcome<VacancyExtraction> {
  ...
  try {
    ...
    return { ok: false, code: "MALFORMED_JSON", message: e instanceof Error ? e.message : String(e) };
  }
  ...
  if (typeof parsed["is_single_real_position"] !== "boolean") {
    return { ok: false, code: "SCHEMA_FAILURE", message: "is_single_real_position must be a boolean" };
  }

  const confidenceRaw = parsed["confidence"];
  const confidence = typeof confidenceRaw === "number" ? confidenceRaw : Number(confidenceRaw);
  if (!Number.isFinite(confidence) || confidence < 0 || confidence > 1) {
    return { ok: false, code: "BUSINESS_RULE_FAILURE", message: "confidence must be a number between 0 and 1" };
  }
  ...
  if (type && !OPPORTUNITY_TYPES.includes(type as (typeof OPPORTUNITY_TYPES)[number])) {
    return { ok: false, code: "BUSINESS_RULE_FAILURE", message: `unsupported opportunity_type: ${type}` };
  }
  ...
  for (const field of ["start_date", "application_deadline"]) {
    ...
    if (v !== null && v !== undefined && v !== "" && !isIsoDate(v)) {
      return { ok: false, code: "BUSINESS_RULE_FAILURE", message: `${field} is not an ISO date: ${String(v)}` };
    }
  }
  ...
  const topics = Array.isArray(parsed["topics"])
    ? (parsed["topics"] as unknown[]).map((t) => nullableString(t, 120)).filter((t): t is string => Boolean(t))
    : [];
  const evidence = Array.isArray(parsed["evidence"])
    ? (parsed["evidence"] as unknown[]).map((t) => nullableString(t, 400)).filter((t): t is string => Boolean(t))
    : [];
  ...
  return {
    ...
    value: {
      ...
      application_deadline: isIsoDate(parsed["application_deadline"]) ? (parsed["application_deadline"] as string) : null,
