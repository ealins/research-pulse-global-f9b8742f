...
export function NvidiaEnginePanel() {
    ...
    try {
      ...
      if (result.model_available) {
        ...
        toast.error("Intelligence engine unavailable", {
          description: `${result.error_code ?? "error"}: ${result.error_message ?? "unknown"}`.slice(0, 300),
        });
      }
      await refetch();
    } catch (e) {
      toast.error("Connection test failed", { description: e instanceof Error ? e.message : String(e) });
    } finally {
      setBusy(null);
    }
  };
  ...
  const onPrecisionTest = async () => {
    ...
    try {
      ...
      setRows(result.rows);
      toast.success("Controlled extraction test finished", { description: `${result.rows.length} pages evaluated` });
      await refetch();
    } catch (e) {
    ...
    <section className="rounded-xl border border-border/60 bg-card/40 p-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            Semantic extraction runs only after the deterministic gates. The model can reject a page but never promote one the
            gate refused, and nothing unvalidated reaches a canonical record.
          </p>
        </div>
      ...
      {error ? (
        <p className="mt-3 rounded border border-destructive/40 bg-destructive/10 p-2 text-xs text-muted-foreground">
          Engine status needs an admin session: {error instanceof Error ? error.message : String(error)}
        </p>
      ) : null}
        ...
        <>
          <dl className="mt-3 grid gap-3 text-xs sm:grid-cols-2">
            ...
            <div>
              ...
              <dd className={`mt-1 font-mono ${data.secret_configured ? "text-primary" : "text-destructive"}`}>
          ...
          <div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-4 lg:grid-cols-7">
            ...
            <Metric label="Avg latency" value={data.avg_latency_ms === null ? "—" : `${data.avg_latency_ms} ms`} />
          </div>

          <p className="mt-2 text-[11px] text-muted-foreground">
            Last success: {data.last_success_at ? new Date(data.last_success_at).toLocaleString() : "none yet"}
            {data.last_error ? ` · Last error: ${data.last_error.code ?? "error"} — ${(data.last_error.message ?? "").slice(0, 160)}` : ""}
          </p>
                ...
                <tbody>
                  {data.recent.map((r) => (
                    <tr key={r.id} className="border-t border-border/40">
                      <td className="px-3 py-2 font-mono text-[11px] text-foreground">{r.operation}</td>
            ...
            <tbody>
              {rows.map((r, i) => (
                <tr key={`${r.url}-${i}`} className="border-t border-border/40 align-top">
                  ...
                  <td className={`px-3 py-2 font-medium ${r.final.startsWith("ACCEPTED") ? "text-primary" : "text-amber-400"}`}>
                    {r.final}
                  </td>
                  <td className="px-3 py-2 tabular-nums">{r.confidence === null ? "—" : r.confidence.toFixed(2)}</td>
                </tr>
              ))}
            </tbody>
          </table>
